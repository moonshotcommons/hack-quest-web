...
const UgcCourseCard: React.FC<UgcCourseCardProp> = ({ isPublic = true, course }) => {
  const status = course.status;
  const { redirectToUrl } = useRedirect();
  const goEdit = (e: MouseEvent<HTMLElement>) => {
    e.stopPropagation();
    redirectToUrl(`${MenuLink.UGC}/${course.id}/creation/${CreationPageKey.Introduction}`);
  };
  const goDetail = () => {
  ...
  const renderButton = () => {
    switch (status) {
      case CourseTab.PUBLISHED:
        return (
          <>
            <Button type="primary" className="button-text-s h-[34px] flex-1 uppercase">
              Preview
            </Button>
            <Button ghost onClick={goEdit} className="button-text-s h-[34px] flex-1 border-neutral-black uppercase">
              edit
            </Button>
          </>
        );
      case CourseTab.DRAFT:
        return (
          <>
            <Button type="primary" onClick={goEdit} className="button-text-s h-[34px] flex-1 uppercase">
              edit & publish
            </Button>
          </>
        );
      case CourseTab.UNDER_REVIEW:
        return (
          <>
            <Button ghost className="button-text-s h-[34px] flex-1 border-neutral-black uppercase">
              revoke
            </Button>
          </>
        );
      case CourseTab.UNPUBLISHED:
        return (
          <>
            <Button type="primary" className="button-text-s h-[34px] flex-1 p-0 uppercase">
              Preview
            </Button>
            <Button ghost onClick={goEdit} className="button-text-s h-[34px] flex-1 border-neutral-black p-0 uppercase">
              edit & publish
            </Button>
          </>
        );
    ...
    >
      ...
      <div className="flex h-[216px] flex-col justify-between p-[16px]">
        <div className="flex flex-col gap-[16px]">
          ...
          {isPublic && <div className="body-s line-clamp-2 text-neutral-medium-gray">{course.description}</div>}
        </div>
        {!isPublic ? (
          <div className="flex flex-col gap-[16px]">
