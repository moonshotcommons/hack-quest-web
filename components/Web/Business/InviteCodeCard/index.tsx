...
const InviteCodeCard: FC<InviteCodeCardProps> = (props) => {
  ...
  const beginnerRewards = useMissionCenterStore((state) => state.missionData.beginnerRewards);

  const { updateMissionData } = useGetMissionData();

  const inviteProgress = useMemo(() => {
    const inviteObject = beginnerRewards.find((item) => item.subType === 'INVITE_USER');
    return {
  ...
  return (
    <div className="w-[380px] rounded-[10px] bg-neutral-white p-5 shadow-md">
      <div className="flex h-fit justify-between ">
        ...
        <div className="flex h-5 w-5 cursor-pointer items-center justify-center" onClick={() => setExpand(!expand)}>
          {expand && <span className="block h-[2px] w-full rounded-full bg-neutral-black"></span>}
        ...
        <>
          <div className="mt-[15px] flex h-[40px] items-center justify-between rounded-[50px] bg-neutral-off-white px-[15px]">
            ...
            <div className="flex h-full gap-[8.5px]">
              ...
              >
                {icons.copyIcon}
                <span className="animate body-xs relative text-neutral-medium-gray hover:text-neutral-black">Copy</span>
              </div>
              <div
                className="flex h-full cursor-pointer items-center gap-[5px]"
          ...
          <div className="mt-5 flex items-center justify-between gap-x-4">
            <div className="flex flex-1 flex-col gap-[13px]">
              <div className="body-s text-neutral-black">
                {`Users you invited (${inviteProgress.progress[0] || 0}/${inviteProgress.progress[1] || 0})`}
              </div>
              <div className="flex w-full gap-[3px]">
                {new Array(inviteProgress.progress[1] || 0).fill('').map((item, index) => {
                  return (
                    <div
                      key={index}
                      className={cn(
...
const icons = {
  descIcon: (
    <svg width="12" height="12" viewBox="0 0 12 12" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <rect x="5.3999" y="4.20001" width="1.2" height="5.4" rx="0.6" fill="white" />
    </svg>
  ),
  inviteIcon: (
