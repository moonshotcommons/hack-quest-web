...
import { CompleteStateType, CourseLessonType, UGCCourseType } from '@/service/webApi/course/type';
...
const LessonSidebar: FC<LessonSidebarProps> = ({ lesson }) => {
  ...
  const getNavbar = (item: any) => {
    if (!course) return;
    const unitName = course.units!.find((unit) => unit.pages.find((page) => page.id === item.key))?.title;
    const navbarData = [{ label: course.title }, { label: unitName }, { label: item.data.title }];
