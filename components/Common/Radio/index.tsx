...
const Radio: FC<RadioProps> = (props) => {
  ...
  return (
    <div className={`${disabled ? 'pointer-events-none cursor-not-allowed' : 'cursor-pointer'}`} onClick={() => onChange?.()}>
      ...
      <span className="block h-[20px] w-[20px] rounded-full border border-solid border-electives-filter-border-color">
        {checked ? <span className={cn(`block h-full w-full scale-[0.7] rounded-full bg-learning-track-line-bg`, className)}></span> : null}
      </span>
    </div>
  );
};
