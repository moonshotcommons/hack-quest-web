...
function SignUpStreak() {
  const signUpStreakList = [1, 2, 3, 4, 5, 6, 7];
  return (
    <div className="bg-mission-center-box h-[220px] rounded-[20px] flex relative overflow-hidden">
      ...
      <div className="absolute left-[135px] flex flex-1 flex-row-center  w-[calc(100%-165px)]  z-[1]  h-full overflow-auto">
        {signUpStreakList.map((item: any) => (
          <div
            className={`flex-shrink-0 w-[119px] h-[142px] ml-3 flex-col-center py-4 justify-between rounded-[20px]  ${
              item < 6 &&
              'bg-mission-center-tab-data text-mission-center-track-color'
            }
            ${
              item === 6 &&
              'w-[117px] h-[140px] border border-dashed border-mission-center-track-border text-mission-center-track-color'
            }
            ${
              item > 6 &&
              'text-mission-center-track-color-r bg-mission-center-track-bg-r'
            }
            `}
            key={item}
          >
            <p className="text-[11px] leading-3">Day 2</p>
            ...
            {item < 6 ? (
              <button className="flex-center w-[79px] h-[40px] text-mission-center-track border border-mission-center-track rounded-[12px] cursor-pointer">
                Claimed
              </button>
            ) : (
              <div
                className={`flex-center w-[79px] h-[40px]  text-mission-center-lock  rounded-[12px] cursor-pointer ${
                  item === 6
                    ? 'bg-mission-center-lock-bg'
                    : 'bg-mission-center-lock-bg-r'
                }`}
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
