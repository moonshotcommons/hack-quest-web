    ...
    >
      <div className="body-s text-neutral-rich-gray">
        {faqs?.map((v, i) => (
          <div
            key={i}
            className={`border-t ${!i ? 'border-transparent pb-[1rem] pt-[0]' : 'border-neutral-medium-gray py-[1.25rem]'}`}
          >
            ...
            <div className={`whitespace-pre-line pt-[1.25rem] ${v.isExpand ? 'block' : 'hidden'}`}>{v.answer}</div>
          </div>
        ))}
      </div>
