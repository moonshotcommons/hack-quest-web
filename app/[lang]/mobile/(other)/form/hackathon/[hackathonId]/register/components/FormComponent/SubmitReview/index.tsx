...
const SubmitReview: FC<Omit<FormComponentProps, 'type' | 'onNext'>> = ({
  ...
  const NameBlock = (
    <div
      ...
      onClick={() => gotoStep(0)}
    >
      <span className="body-m-bold flex-1 text-left">{isEmpty(submissionType.team) ? 'Name' : 'Team Name'}</span>
      <span className="body-s flex-1 text-left">
  ...
  const ContractInfoBlock = (
    <div className="flex flex-col gap-2 rounded-[8px] border border-neutral-light-gray bg-neutral-white px-6 py-3">
      ...
      <div className="body-s flex items-center justify-between text-neutral-off-black">
        <span>Email</span>
  ...
  const BioBlock = (
    <div className="flex min-h-[150px] flex-col gap-2 rounded-[8px] border border-neutral-light-gray bg-neutral-white px-6 py-3">
      ...
      <p className="body-s text-left text-neutral-off-black">{bio}</p>
    </div>
  );
