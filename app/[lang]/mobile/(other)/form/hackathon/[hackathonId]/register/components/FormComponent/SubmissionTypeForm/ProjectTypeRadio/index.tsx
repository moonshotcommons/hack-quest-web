...
const ProjectTypeRadio = ({ form, submissionType, hackathonId }: ProjectTypeRadioProps) => {
  return (
    <div className="flex w-full flex-col gap-4">
      <p className="body-m text-left text-neutral-off-black">Please choose the project type you want to submit</p>
      <div className="flex w-full justify-between gap-4">
        <div
          onClick={() => {
            if (hackathonId === HackathonPartner.Hack4Bengal) return;
          ...
          className={cn(
            `body-s-bold flex  w-full cursor-pointer items-center justify-center gap-3 rounded-[8px] border-[3px] border-neutral-off-white p-4 tracking-tight`,
            form.watch('type') === 'Solo Project'
              ? 'border-yellow-dark bg-yellow-extra-light shadow-[0px_0px_8px_0px_rgba(249,216,28,0.30)]'
              : '',
            !!Object.keys(submissionType.team || {}).length || hackathonId === HackathonPartner.Hack4Bengal
              ? '!cursor-not-allowed'
              : ''
          )}
          title={
        ...
        <div
          ...
          className={cn(
            `body-s-bold flex w-full cursor-pointer items-center justify-center gap-3 rounded-[8px] border-[3px] border-neutral-off-white p-4 tracking-tighter`,
