...
interface ProjectDetailSectionFormProps {
  ...
  projectDetail: ProjectSubmitStateType['ProjectDetail'];
}

const ProjectDetailSectionForm: FC<ProjectDetailSectionFormProps & CommonFormComponentProps> = ({
  sectionConfig,
  projectDetail,
  projectId,
  isSubmit,
  ...
  const { runAsync: submitRequest, loading } = useRequest(
    async (values: Record<string, string>, isExit = false) => {
      const { nextStep } = getHackathonStepInfo(
        ...
        SubmissionSectionType.ProjectDetail
      );
      ...
      const state = {
        ...
        status: isExit ? SubmissionSectionType.ProjectDetail : nextStep.type
      };
  ...
  function onSubmit(values: Record<string, string>) {
    debugger;
    const isSame = isEqual(values, projectDetail);
    if (isSame) {
      onNext();
      return;
    }
    ...
    for (let key in projectDetail) {
      switch (key) {
        case 'id':
          break;
        case 'fields':
          for (let k in projectDetail[key] || {}) {
            projectDetail[key] && form.setValue(k, projectDetail[key]![k].value);
          }
        default:
          form.setValue(key, projectDetail[key]);
          break;
      }
    }

    const propValues = Object.values(projectDetail);
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col gap-6">
          ...
          <div className="flex justify-end gap-4">
            <Button
              ...
              disabled={hackathonSteps[0].type === SubmissionSectionType.ProjectDetail}
              onClick={onBack}
            >
              Back
            </Button>
...
export default ProjectDetailSectionForm;
