...
import { CompleteStateType, CourseLessonType, UnitPagesListType } from '@/service/webApi/course/type';
import { useCourseStore } from '@/store/zustand/courseStore';
import { useRequest } from 'ahooks';
import { useEffect, useState } from 'react';

const formateDropdownData = (data: UnitPagesListType[], lesson: CourseLessonType) => {
  ...
  const newData: UnitPagesListType[] = data.map((unit, index) => {
    ...
    const newUnit = {
      ...
      pages: unit.pages?.map((page, pageIndex) => {
        const newPage = {
          ...page,
          disable: page.state === CompleteStateType.NOT_STARTED && prevLessonState !== CompleteStateType.COMPLETED
        };
...
export const useUnitNavList = (lesson: CourseLessonType) => {
  ...
  const setUnitsLessonsList = useCourseStore((state) => state.setUnitsLessonsList);

  const { run, refresh } = useRequest(
    async () => {
      const data = await webApi.courseApi.getCourseUnitsAndPages(lesson?.courseId);
      if (data) {
